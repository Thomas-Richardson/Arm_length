...
#library(gender)
#=read_csv('MMA_data.csv') # read in unsexed data

#data_MMA$sex_weight = 'unknown'
...
#sex=gender(data_MMA$First_name,years = c(1969,2001)) # classify sex from first name using name databases from 1969-2001
...
#data_MMA$sex[is.na(data_MMA$sex)]='unknown'

#write_csv(data_MMA,'MMA_data_sexed_.csv')

# note that it will fail to identify around 200 fighters which whose sex I then googled.
...
cor.test(women$Armspan,women$Height) # correlation of armspand and height in women
cor.test(men$Armspan,men$Height)       # correlation of armspand and height in men
cor.test(data$AHR,data$Weight)      # correlation between armspan to height ratio and weight, to test for positive allometry 
cor.test(data$AHR,data$Height)

lm(Armspan~Height+sex+Weight,data) %>% summary() # multiple regression testing sex differences in armspan controlling for other variables
...
mod = lm(Win_Percentage~Armspan+Height+sex+Weight,data) # multiple regression predicting win percentage from height weight armspan and sex
